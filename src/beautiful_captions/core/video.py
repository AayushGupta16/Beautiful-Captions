...
class Video:
    ...
    def __init__(
        self, video_path: Union[str, Path], config: Optional[CaptionConfig] = None
    ):
    ...
    async def transcribe(
        ...
        if isinstance(service, str):
            ...
            service = cast(ServiceType, service)
        ...
        custom_censored_words = getattr(
            self.config.style, "custom_censored_words", None
        )
    ...
    def add_captions(
        ...
        if srt_input_path:
            ...
            raise ValueError(
                "No caption content available. Provide either srt_path, srt_content, or run transcribe() first."
            )

        srt_content = srt_content or self._srt_content

        # Apply styling if enabled
        if add_styling and self.config.diarization and self.config.diarization.enabled:
            _srt_content = srt_content if srt_content else ""
            _keep_speaker_labels = self.config.diarization.keep_speaker_labels
            _max_words_per_line = (
                self.config.style.max_words_per_line if self.config.style else 1
            )
            _font = self.config.style.font if self.config.style else None
        ...
        combine_video_subtitles(self.video_path, self._ass_path, output_path, cuda)
