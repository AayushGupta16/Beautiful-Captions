...
def create_transcription_service(
    service: ServiceType, api_key: str
...
async def add_subtitles(
    ...
    if config is None:
        ...
        if style is not None:
            if isinstance(style, str):
                ...
                raise TypeError(
                    "Style must be a string, dictionary, or StyleConfig object"
                )
        else:
    ...
    with Video(video_path, config) as video:
        ...
        if srt_output_path:
            ...
            _video_str_content = video._srt_content if video._srt_content else ""
            _video_config_diarization_colors = (
                video.config.diarization.colors if video.config.diarization else None
            )
            _enable_speaker_colors = (
                video.config.diarization.enabled if video.config.diarization else False
            )
            _keep_speaker_labels = (
                video.config.diarization.keep_speaker_labels
                if video.config.diarization
                else False
            )
            _max_words_per_line = (
                video.config.style.max_words_per_line if video.config.style else 1
            )
            _font = video.config.style.font if video.config.style else None
            styled_srt = style_srt_content(
                _video_str_content,
                _video_config_diarization_colors,
                encode_speaker_colors=_enable_speaker_colors,
                keep_speaker_labels=_keep_speaker_labels,
...
async def subtitles_from_srt(
    ...
    if config is None and style is not None:
        ...
        config = CaptionConfig(
            style=style_config,
            diarization=DiarizationConfig(
                enabled=True, keep_speaker_labels=False, colors=diarization_colors
            ),
        )
...
async def extract_subtitles(
    ...
    with Video(video_path, config) as video:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(video._srt_content if video._srt_content else "")
...
def caption_stream(
    ...
    with Video(video_path, config) as video:
        return video.add_captions(srt_content=srt_content, output_path=output_path)
